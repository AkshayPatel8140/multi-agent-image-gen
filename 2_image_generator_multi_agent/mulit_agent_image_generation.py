...
def prompt_engineer_agent(state: MessagesState) -> Command[Literal["image_node", END]]:
    ...
    updated_messages = state["messages"] + [AIMessage(content=response_text, name="prompt_engineer")]
...
def image_generator_agent(state: MessagesState) -> Command[Literal[END, "prompt_node"]]:
    ...
    updated_messages = state["messages"] + [AIMessage(content=response, name="image_agent")]
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("Example idea:")
    print("A cozy steampunk library interior with warm light and brass, isometric view, ultra-detailed")

    user_idea = input("\n💡 Your idea: ").strip()

    # Run the agent system
    final_image_path = None
    for event in graph.stream({"messages": [HumanMessage(content=user_idea)]}, stream_mode="values"):
