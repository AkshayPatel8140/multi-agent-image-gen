...
def openai_image_generator(prompt: str, size: str = "1024x1024") -> Image.Image:
    ...
    try:
        ...
        result = client.images.generate(model=IMAGE_MODEL, prompt=prompt, size=size, n=1, quality="standard")
...
def check_style_compliance(prompt: str, brand: BrandKBItem) -> Dict[str, Any]:
    ...
    style_keywords = ["modern", "clean", "professional", "innovative", "friendly", "trustworthy"]
...
def run_brand_verification(prompt: str, image: Image.Image, brand: BrandKBItem) -> Dict[str, Any]:
    """
    ...
    overall_compliant = (palette_check["compliant"] and style_check["overall_style_compliant"])
...
def save_verification_report(
    ...
    report = {
        ...
        "user_input": {
            ...
            "user_position": (
                state["preferred_position"]
                if state["preferred_position"]
                else brand.default_logo_position
            ),
        },
        "generation_details": {
            "prompt": prompt,
        ...
        "summary": {
            ...
            "style_compliant": verification_result["style_compliance"][
                "overall_style_compliant"
            ],
        },
    }

    # Save to verification directory
    report_path = os.path.join(VERIFICATION_DIR, f"verification_report_{timestamp}.json")
...
class State(TypedDict):
    ...
    size: str = "1024x1024"
    preferred_position: Optional[str] = "top-center"  # Can override brand default
...
def brand_node(state: State) -> Command[Literal["prompt_node"]]:
    ...
    if company not in KB:
        raise ValueError(f"Unknown company key: '{company}'. Add it to {os.path.join(KB_DIR, 'brand_kb.json')}.")
    brand_data = KB[company]

    # Inject a brand summary message so downstream node can use it
    brand_msg = AIMessage(content=f"[BRAND_CONTEXT]\n{brand_context_text(brand_data)}", name="brand_kb")
    return Command(update={"messages": state["messages"] + [brand_msg]}, goto="prompt_node")
...
def prompt_node(state: State) -> Command[Literal["image_node", END]]:
    ...
    msgs = [SystemMessage(content=PROMPT_ENGINEER_SYS), AIMessage(content=brand_text), *state["messages"]]
    ai_response = llm.invoke(msgs)
    text = ai_response.content.strip().replace("\n", " ")
    if not text.startswith("FINAL PROMPT:"):
        text = "FINAL PROMPT: " + text.strip()

    return Command(
        update={"messages": state["messages"] + [AIMessage(content=text, name="prompt_engineer")]},
        goto="image_node",
    )
...
def compose_node(state: State) -> Command[Literal["verify_node"]]:
    ...
    position = state["preferred_position"]

    if position is None:
        position = brand.default_logo_position
    ...
    return Command(update={"messages": state["messages"] + [out_msg]}, goto="verify_node")
...
def verify_node(state: State) -> Command[Literal[END]]:
    ...
    report_path = save_verification_report(verification_result, final_image_path, final_prompt, state)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--idea",
        required=True,
        help="Your image idea description",
    )
    parser.add_argument(
        "--size",
        default="1024x1024",
    ...
    if final_image_path and verification_report_path:
        ...
        print(f"🎯 Brand Compliance: {'✅ COMPLIANT' if brand_compliant else '❌ NOT COMPLIANT'}")
        ...
        if not brand_compliant:
            ...
            print("   Check the verification report for details on what needs improvement.")
    else:
